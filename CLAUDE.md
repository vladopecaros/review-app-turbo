...
- Run typecheck and lint:fix for **both** apps before every commit — no exceptions:
  ```
  cd apps/api && npm run typecheck && npm run lint:fix
  cd apps/web && npm run typecheck && npm run lint:fix
  ```
