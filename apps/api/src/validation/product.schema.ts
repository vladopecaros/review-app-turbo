...
export const bulkProductItemSchema = z.object({
  externalProductId: z.string().trim().min(1, 'External product id is required'),
  name: z.string().trim().min(3, 'Product name must be more than 3 characters long'),
  slug: z.string().trim().min(3, 'Product slug must be more than 3 characters long'),
  description: z.string().optional(),
  active: z.boolean().optional(),
  metadata: z.record(z.string(), z.unknown()).optional(),
});
