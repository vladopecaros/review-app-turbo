    ...
    <main className="grid min-h-screen grid-cols-1 bg-[color:var(--bg)] lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <section className="hidden border-r border-white/5 p-8 lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="space-y-4">
          <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--text-muted)]">{t('auth.login.eyebrow')}</p>
          <h1 className="font-display text-4xl font-semibold tracking-tight">{t('auth.login.title')}</h1>
          <p className="max-w-md text-[color:var(--text-muted)]">{t('auth.login.subtitle')}</p>
        </div>
      ...
      <section className="flex items-center justify-center px-4 py-10 sm:px-6">
        <div className="surface-panel w-full max-w-md p-6 sm:p-7">
          ...
          <div className="mb-6 space-y-2">
            <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--text-muted)]">{t('auth.login.eyebrow')}</p>
            <h2 className="font-display text-2xl font-semibold tracking-tight">{t('auth.login.title')}</h2>
            <p className="text-sm text-[color:var(--text-muted)]">{t('auth.login.subtitle')}</p>
          </div>
          <LoginForm />
        </div>
      </section>
